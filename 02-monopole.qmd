...
Pour comprendre ce que la contrainte d'information coûte au monopoleur, il est utile de partir du cas de référence où il *pourrait* identifier les types. Il pratiquerait alors une discrimination parfaite : il proposerait à chaque consommateur de type 1 la quantité $q_1^o$ telle que $P_1(q_1^o)=c$, et à chaque consommateur de type 2 la quantité $q_2^o$ telle que $P_2(q_2^o)=c$. La situation est représentée sur la @fig-discri2-parfaite. Il ferait payer à chaque consommateur de type 1 la totalité du surplus que lui procure la consommation de $q_1^o$ unités, c'est-à-dire la surface $A$, et à chaque consommateur de type 2 la totalité du surplus que lui procure $q_2^o$ unités, c'est-à-dire la surface $A+B+C$. Comme dans le cas standard de la discrimination parfaite, le monopole produit les quantités qui maximisent le surplus total (Pareto-optimales) et s'approprie la totalité du surplus des consommateurs.
...
Cependant, le monopoleur discriminant au deuxième degré ne sait pas qui est de type 1 et qui est de type 2. S'il proposait ces mêmes menus $(q_1^o, A)$ et $(q_2^o, A+B+C)$, les consommateurs de type 2 choisiraient en réalité le menu destiné au type 1 : il leur procure un surplus $B>0$, alors que le menu qui leur est destiné leur laisse un surplus nul. C'est précisément pour empêcher ce type de déviation que le monopoleur doit concevoir ses menus sous les contraintes formalisées ci-dessous.

#### Notations : menus et valorisation totale

Un menu est un couple $(q, T)$ : le consommateur qui l'accepte reçoit $q$ unités du bien et paie un montant total (un *tarif*) $T$. Notons

$$
V_i(q) = \int_0^q P_i(x)\,\mathrm{d}x
$$

la **valorisation totale** (la disposition à payer totale) d'un consommateur de type $i$ pour $q$ unités : c'est l'aire sous sa fonction de demande inverse entre $0$ et $q$. Par construction, $V_i'(q)=P_i(q)$ : la valorisation marginale d'une unité supplémentaire est bien le prix de réserve $P_i(q)$.
...
Comme $P_2(q)>P_1(q)$ pour tout $q$, on a $V_2(q)>V_1(q)$, et surtout l'écart $V_2(q)-V_1(q)$ est **croissant** en $q$ : plus la quantité (ou la qualité) est élevée, plus l'avantage du type 2 sur le type 1 à la consommer est grand. C'est cette propriété (dite de *simple croisement*) qui rend la discrimination possible.

#### Le programme du monopole

Le monopoleur propose deux menus, $(q_1,T_1)$ destiné au type 1 et $(q_2,T_2)$ destiné au type 2. Avec autant de consommateurs de chaque type, son profit (par paire de consommateurs, un de chaque type) s'écrit

$$
\max_{q_1,T_1,q_2,T_2}\ \Pi = \big(T_1 - c\,q_1\big) + \big(T_2 - c\,q_2\big).
$$

Puisqu'il ne peut pas identifier les types, il ne peut pas *imposer* à chaque consommateur le menu qui lui est destiné : il doit concevoir les menus de sorte que chacun **choisisse librement** le bon. Deux séries de contraintes en découlent.
...
Résoudre le programme revient à déterminer *lesquelles* de ces quatre contraintes sont saturées (vérifiées avec égalité) à l'optimum. On remarquera que ce raisonnement ne dépend **pas** des proportions de chaque type : il repose uniquement sur la structure des contraintes.

- **(CP$_1$) est saturée.** Si le type 1 conservait un surplus strictement positif, le monopoleur pourrait augmenter *simultanément* $T_1$ et $T_2$ d'un même petit montant : les deux contraintes d'incitation, qui ne portent que sur des *différences* de tarifs, sont préservées, et le profit augmente. Il continue jusqu'à ce que le surplus du type 1 soit nul :
$$
T_1 = V_1(q_1).
$$

- **(CI$_2$) est saturée.** Une fois $T_1$ fixé, le monopoleur augmente $T_2$ autant que possible pour extraire le surplus du type 2. Il est arrêté par la contrainte d'incitation du type 2 : au-delà, le type 2 préférerait choisir le menu 1. À l'optimum le type 2 est donc *indifférent* entre les deux menus :
$$
...
- **(CP$_2$) et (CI$_1$) sont vérifiées automatiquement** (elles sont *non saturées*). Pour (CP$_2$) : le surplus du type 2 vaut $V_2(q_1)-V_1(q_1)\geq 0$, donc il participe. Pour (CI$_1$) : comme $V_2(q_2)-V_2(q_1)\geq V_1(q_2)-V_1(q_1)$ (propriété de simple croisement), on vérifie que le type 1 ne gagne rien à choisir le menu 2 ; il n'est donc jamais tenté de dévier. Il est inutile de les imposer.
...
$$
\frac{\partial \Pi}{\partial q_2}=0 \ \Leftrightarrow\ P_2(q_2^m)-c=0 \ \Leftrightarrow\ \boxed{P_2(q_2^m)=c}
$$

$$
\frac{\partial \Pi}{\partial q_1}=0 \ \Leftrightarrow\ \big(P_1(q_1^m)-c\big) - \big(P_2(q_1^m)-P_1(q_1^m)\big)=0 \ \Leftrightarrow\ \boxed{P_1(q_1^m)-c = P_2(q_1^m)-P_1(q_1^m)}
$$ {#eq-cpo2e}

Ces deux conditions résument tout le résultat.

1. **Pas de distorsion « en haut ».** La quantité offerte au type 2 est celle de la discrimination parfaite, $q_2^m=q_2^o$, définie par $P_2(q_2^m)=c$. Le monopoleur produit la quantité efficace pour le type 2 ; il se contente d'en réduire le tarif pour laisser la rente $B$.

2. **Distorsion « en bas ».** La quantité offerte au type 1 est *réduite* en deçà de son niveau efficace $q_1^o$ (défini par $P_1(q_1^o)=c$). En effet le membre de droite de l'@eq-cpo2e est strictement positif, donc $P_1(q_1^m)>c$, c'est-à-dire $q_1^m<q_1^o$. L'arbitrage est le suivant : réduire d'une unité $q_1$ coûte $P_1(q_1)-c$ de surplus sur le consommateur de type 1, mais réduit d'autant la rente laissée au consommateur de type 2, $P_2(q_1)-P_1(q_1)$. Le monopoleur réduit $q_1$ tant que le second effet l'emporte, et s'arrête à l'égalité de l'@eq-cpo2e.
...
L'hypothèse de proportions égales n'a servi qu'à écrire le profit « par paire de consommateurs ». Relâchons-la : notons $\lambda_1$ et $\lambda_2$ les proportions de chaque type dans la population ($\lambda_1+\lambda_2=1$). Le profit du monopoleur s'écrit désormais comme une **moyenne pondérée** par ces proportions :

$$
\Pi = \lambda_1\big(T_1 - c\,q_1\big) + \lambda_2\big(T_2 - c\,q_2\big).
$$

Deux points ne changent pas. D'une part, **les tarifs saturés sont les mêmes** : le choix des contraintes saturées (CP$_1$ et CI$_2$) reposait uniquement sur la structure des contraintes, pas sur les proportions. On a donc toujours $T_1 = V_1(q_1)$ et $T_2 = V_2(q_2) - [V_2(q_1)-V_1(q_1)]$. D'autre part, **la quantité du type 2 reste efficace** : en dérivant le profit par rapport à $q_2$, le facteur $\lambda_2$ se met en facteur et disparaît, si bien que la condition reste $P_2(q_2^m)=c$, soit $q_2^m=q_2^o$. Le résultat de « pas de distorsion en haut » est donc robuste.

Ce qui change, c'est la **condition sur $q_1$**. Le profit réduit devient
...
$$
\frac{\partial \Pi}{\partial q_1}=0 \ \Leftrightarrow\ \lambda_1\big(P_1(q_1^m)-c\big) - \lambda_2\big(P_2(q_1^m)-P_1(q_1^m)\big)=0 \ \Leftrightarrow\ \boxed{P_1(q_1^m)-c = \frac{\lambda_2}{\lambda_1}\big(P_2(q_1^m)-P_1(q_1^m)\big)}
$$ {#eq-cpo2e-lambda}
...
Le monopoleur pondère donc chaque effet par la taille du groupe concerné : il distord $q_1$ tant que le gain sur les rentes du type 2 ($\lambda_2$) l'emporte sur la perte d'efficacité sur le type 1 ($\lambda_1$). Il en résulte que **plus les consommateurs à forte valorisation sont nombreux** (grand $\lambda_2/\lambda_1$), **plus le monopoleur dégrade l'offre destinée au type 1** : la rente à leur concéder pèse d'autant plus lourd qu'ils sont nombreux, et il est prêt à sacrifier davantage d'efficacité sur le type 1 pour la réduire. À la limite, si les consommateurs de type 1 sont très rares, le monopoleur peut préférer ne pas les servir du tout et se concentrer sur le type 2.
