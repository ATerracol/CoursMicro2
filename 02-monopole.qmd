...
Comme $P_2(q)>P_1(q)$ pour tout $q$, on a $V_2(q)>V_1(q)$, et surtout l'écart $V_2(q)-V_1(q)$ est **croissant** en $q$ : plus la quantité (ou la qualité) est élevée, plus l'avantage du type 2 sur le type 1 à la consommer est grand.
C'est cette propriété (dite de *simple croisement*) qui rend la discrimination possible.
...
$$\max_{q_1,T_1,q_2,T_2}\ \Pi = \big(T_1 - c\,q_1\big) + \big(T_2 - c\,q_2\big).$$
...
**Contraintes de participation** (chaque type doit préférer son menu à ne rien acheter, surplus nul) :
...
On remarquera que ce raisonnement ne dépend **pas** des proportions de chaque type : il repose uniquement sur la structure des contraintes.
...
- **(CI**$_2$) est saturée.
  Une fois $T_1$ fixé, le monopoleur augmente $T_2$ autant que possible pour extraire le surplus du type 2.
...
- **(CP**$_2$) et (CI$_1$) sont vérifiées automatiquement (elles sont *non saturées*).
  ...
  Pour (CI$_1$) : comme $V_2(q_2)-V_2(q_1)\geq V_1(q_2)-V_1(q_1)$ (propriété de simple croisement), on vérifie que le type 1 ne gagne rien à choisir le menu 2 ; il n'est donc jamais tenté de dévier.
...
1.  **Pas de distorsion « en haut ».** La quantité offerte au type 2 est celle de la discrimination parfaite, $q_2^m=q_2^o$, définie par $P_2(q_2^m)=c$.
    Le monopoleur produit la quantité efficace pour le type 2 ; il se contente d'en réduire le tarif pour laisser la rente $B$.
...
\end{array}$$
 {#eq-cpo3e} L'@eq-cpo3e nous dit que le monopole réparti la quantité totale produite de manière à égaliser les recettes marginales entre les deux groupes.
