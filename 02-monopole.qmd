...
On peut donc écrire :
$$R_m(q) = P(q)\left(1 - \frac{1}{|\varepsilon_{q/p}(q)|}\right)$$ {#eq-rm}
...
$$R_m(q^*) = P(q^*)\left(1 - \frac{1}{|\varepsilon_{q/p}(q^*)|}\right) = C_m(q^*) $$ {#eq-rmcm}
...
$$L=\frac{P(q^*) - C_m(q^*)}{P(q^*)} $$ {#eq-Lerner}
:::
...
La condition du premier ordre de l'@eq-cpo devient :
$$\begin{array}{crcl}
&(R(q)-C(q) +sq)'(q^*) &=& 0\\
...
Une vision **alternative** à la diminution du coût marginal est de considérer que la subvention augmente la recette marginale de $s$ :
$$R_m(q^*) + s = C_m(q^*)$$
...
Comme $R_m(q) = P(q) + qP'(q)$, elle vaut aussi :
$$s^* = -q^*_{CPP}\,P'(q^*_{CPP}) > 0$$
:::
...
En utilisant l'expression de la recette marginale de l'@eq-rm, on peut aussi écrire la subvention optimale en fonction de l'élasticité :
$$\frac{s^*}{P(q^*_{CPP})}=\frac{1}{\left|\varepsilon_{q/p}(q^*_{CPP})\right|}$$
On retrouve la même expression que l'indice de Lerner de l'@eq-Lerner, évaluée cette fois en $q^*_{CPP}$ : *plus la demande est inélastique, plus le monopole restreint sa production, et plus la subvention nécessaire est élevée.*
...
Sans régulation, le monopole égalise $1-2q = q$, soit :
$$q^*_m = \frac{1}{3}, \qquad p^*_m = \frac{2}{3}, \qquad \pi(q^*_m)=\frac{1}{6}$$

L'optimum social est en $P(q)=C_m(q)$, soit $1-q = q$ :
$$q^*_{CPP} = \frac{1}{2}, \qquad p^*_{CPP} = \frac{1}{2}$$

La subvention optimale de l'@eq-subv-opti vaut :
$$s^* = P\left(\tfrac{1}{2}\right) - R_m\left(\tfrac{1}{2}\right) = \frac{1}{2} - 0 = \frac{1}{2}$$
...
Le profit du monopole subventionné vaut alors :
$$\pi = R(q^*_{CPP}) - C(q^*_{CPP}) + s^*q^*_{CPP} = \frac{1}{4}-\frac{1}{8}+\frac{1}{4} = \frac{3}{8}$$
:::

##### Le coût de la subvention

La subvention n'est pas gratuite : le régulateur verse au monopole
$$S = s^*\,q^*_{CPP}$$
Dans l'@exm-subv-opti, cela représente $S = \tfrac{1}{2}\times\tfrac{1}{2} = \tfrac{1}{4}$.
...
Le régulateur équilibre son budget en prélevant exactement ce qu'il verse :
$$T = S = s^*q^*_{CPP}$$ {#eq-T-financement}
...
Elle est réalisable, car elle reste inférieure au profit du monopole subventionné, qui vaut $\tfrac{3}{8}$ :
$$T = \frac{1}{4} \leq \frac{3}{8} = \pi(q^*_{CPP})$$

Le profit net du monopole après subvention et taxe vaut :
$$\frac{3}{8}-\frac{1}{4} = \frac{1}{8} > 0$$
Le monopole continue donc de produire.
:::

Le profit net obtenu dans l'@exm-financement n'est pas un nombre quelconque.
En remplaçant $T$ par sa valeur de l'@eq-T-financement, le profit du monopole régulé s'écrit :
$$\pi_{\text{net}} = \underbrace{R(q^*_{CPP}) - C(q^*_{CPP}) + s^*q^*_{CPP}}_{\text{profit avec subvention}} - \underbrace{s^*q^*_{CPP}}_{T} = R(q^*_{CPP}) - C(q^*_{CPP})$$
...
Le monopole n'accepte de produire que si $\pi_{\text{net}}\geq 0$, c'est-à-dire :
$$R(q^*_{CPP}) \geq C(q^*_{CPP}) \Leftrightarrow p^*_{CPP} \geq C_M(q^*_{CPP})$$
...
Le régulateur choisit cette fois le *taux* $t$ qui lui rapporte le montant $S$ :
$$t\,\pi(q^*_{CPP}) = S = s^*q^*_{CPP} \quad\Leftrightarrow\quad t^* = \frac{s^*q^*_{CPP}}{R(q^*_{CPP}) - C(q^*_{CPP}) + s^*q^*_{CPP}}$$

où le profit imposé $\pi(q^*_{CPP})$ inclut bien la subvention reçue, qui fait partie des recettes du monopole.

Dans l'@exm-financement, cela donne :
$$t^* = \frac{1/4}{3/8} = \frac{2}{3}$$
...
- **(CP**$_1$) est saturée.
  ...
  Il continue jusqu'à ce que le surplus du type 1 soit nul :
  $$T_1 = V_1(q_1).$$
...
- **(CI**$_2$) est saturée.
  ...
  À l'optimum le type 2 est donc *indifférent* entre les deux menus :
  $$V_2(q_2) - T_2 = V_2(q_1) - T_1 \quad\Longrightarrow\quad T_2 = V_2(q_2) - \big[V_2(q_1) - V_1(q_1)\big].$$
  Le type 2 ne paie donc pas toute sa valorisation : il conserve une **rente informationnelle** égale à $V_2(q_1)-V_1(q_1)$, c'est-à-dire l'avantage qu'il tirerait à consommer la quantité destinée au type 1.
